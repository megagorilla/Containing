				...
				if (m.shouldDespawn) {
					...
					for (ContainerData c : m.containers) {
						Container container = new Container(
								ContainingClient.quality, c.containerID);
						container.setLocalTranslation(Container.width
								* c.Location.y - 3, Container.height
								* c.Location.z, Container.length * c.Location.x
								- 30);
						ship.addContainer(container);
					}
					ship.setLocalTranslation(400, 0,
							200f * ContainingClient.barges.size() + 600);
		...
		ContainingClient.instance.enqueue(new Callable<Object>() {
			public Object call() throws Exception {
				if (m.shouldDespawn) {
					...
					SeaShip ship = new SeaShip(ContainingClient.quality,
							m.seaShipID);
					...
					for (ContainerData c : m.containers) {
						Container container = new Container(
								ContainingClient.quality, c.containerID);
						container
								.setLocalTranslation(Container.width
										* c.Location.z - 20, Container.height
										* c.Location.y, Container.length
										* c.Location.x);

						ship.addContainer(container);
					}

					ship.setLocalTranslation(
							800f * ContainingClient.seaShips.size(), 0, 930);
		...
		ContainingClient.instance.enqueue(new Callable<Object>() {
			public Object call() throws Exception {
				if (m.shouldDespawn) {
					...
					if (m.containerID != 0) {
						Container container = new Container(
								ContainingClient.quality, m.containerID);
